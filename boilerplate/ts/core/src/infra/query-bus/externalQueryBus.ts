...
export class ExternalQueryBus extends QueryBus {
  override async query<T>(query: IQuery, errorTypes?: TErrors): Promise<T> {
    // eslint-disable-next-line no-async-promise-executor
    return new Promise(async (resolve, reject) => {
      console.log(
        'sendAndGetResponse: query.metadata.responseTopic',
        query.metadata?.responseTopic,
      );
      ...
      console.log('sendAndGetResponse: before publishing query', query.queryTopic);
