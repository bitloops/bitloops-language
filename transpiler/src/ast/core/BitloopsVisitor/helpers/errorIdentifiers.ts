...
export const errorIdentifiersVisitor = (
  ...
  const errors: ErrorIdentifierNode[] = allErrorsAndBitOrs.filter((item) => item !== undefined);

  const errorsNode = new ErrorIdentifiersNodeBuilder().withErrors(errors).build();
