...
export const servicePortToTargetLanguage = (
  variable: TServicePort,
): TTargetDependenciesTypeScript => {
  const { ServicePort } = variable;
  const { methodDefinitionList, ServicePortIdentifier } = ServicePort;

  let res = `export interface ${ServicePortIdentifier} `;
  res += '{';
