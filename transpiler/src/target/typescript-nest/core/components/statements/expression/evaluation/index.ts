...
const evaluationToTargetLanguage = (variable: TEvaluation): TTargetDependenciesTypeScript => {
  ...
  if (EvaluationTypeIdentifiers.isDomainEventEvaluation(evaluation)) {
    return modelToTargetLanguage({
      type: BitloopsTypesMapping.TDomainEventEvaluation,
      value: evaluation,
    });
  }

  if (EvaluationTypeIdentifiers.isReadModelEvaluation(evaluation)) {
    return modelToTargetLanguage({
      type: BitloopsTypesMapping.TReadModelEvaluation,
      value: evaluation,
    });
  }
  if (EvaluationTypeIdentifiers.isPackageEvaluation(evaluation)) {
    return modelToTargetLanguage({
      type: BitloopsTypesMapping.TPackageEvaluation,
      value: evaluation,
    });
  }
