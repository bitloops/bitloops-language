...
const domainServicePrivateMethod = (methodInfo: TPrivateMethod): TTargetDependenciesTypeScript => {
  ...
  const returnType = `Promise<${mappedReturnType.output}>`;
  ...
  return {
    output: `private async ${staticKeyWord} ${methodName}${parametersString}: ${returnType} { ${methodStatements} }`,
    dependencies: [
...
export { domainServicePrivateMethod };
