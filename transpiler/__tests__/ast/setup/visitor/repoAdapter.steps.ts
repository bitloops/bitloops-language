...
const feature = loadFeature('__tests__/ast/setup/visitor/repoAdapter.feature');

defineFeature(feature, (test) => {
  test('Valid Repo Adapter Definition', ({ given, when, then }) => {
    let blString;
    let modelOutput;
    let result;

    given(/^A valid Repo Connection and Repo Adapter (.*) string$/, (arg0) => {
      blString = d(arg0);
    });
