...
import { validIntegrationEventHandlersTestCases } from './mocks/integration-event-handler/index.js';
import { isIntermediateASTValidationErrors } from '../../../src/ast/core/guards/index.js';

const BOUNDED_CONTEXT = 'Hello World';
const MODULE = 'core';

describe('Integration event handler declaration is valid', () => {
  let resultTree: IntermediateASTTree;

  const parser = new BitloopsParser();
  const intermediateParser = new IntermediateASTParser();

  validIntegrationEventHandlersTestCases.forEach((testCase) => {
    test(`${testCase.description}`, () => {
      ...
      const nodes = resultTree.getRootChildrenNodesByType(
