...
const feature = loadFeature(
  './__tests__/features/bitloopsLanguageToModel/modelFragments/thisDeclaration.feature',
);

defineFeature(feature, (test) => {
  test('ThisDeclaration is valid', ({ given, when, then }) => {
    let blString;
    let modelOutput;
    let result;

    given(/^A valid thisDeclaration (.*) string$/, (arg0) => {
