...
describe('Query declaration is valid', () => {
  ...
  validQueryTestCases.forEach((testQuery) => {
    test(`${testQuery.description}`, () => {
        ...
        core: [
          {
            ...
            module: MODULE,
            fileId: testQuery.fileId,
            fileContents: testQuery.inputBLString,
          },
        ],
      });
      ...
      const queryNodes = resultTree.getRootChildrenNodesByType(BitloopsTypesMapping.TQuery);
      const value = queryNodes[0].getValue();

      expect(value).toMatchObject(testQuery.queryDeclaration.getValue());
    });
  });
});
