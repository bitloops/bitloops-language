...
import {
  VALID_INTEGRATION_EVENT_TEST_CASES,
  contextInfo,
} from './mocks/integration-event/index.js';

describe('Valid integration event declaration test cases', () => {
  const formatterConfig = null;
  const language = SupportedLanguages.TypeScriptNest;
  const { boundedContextName, moduleName } = contextInfo;

  VALID_INTEGRATION_EVENT_TEST_CASES.forEach((testCase) => {
    let resultCore: TTargetCoreFinalContent[];

    it(`${testCase.description}`, () => {
      // given
      const tree = new IntermediateASTTree(new IntermediateASTRootNode());
      const integrationEvent = testCase.integrationEvent;

      tree.insertChild(integrationEvent);
