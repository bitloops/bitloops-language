...
import { VALID_READ_MODEL_EVALUATION_TEST_CASES } from './mocks/expression/evaluation.js';

describe('Valid read model evaluation test cases', () => {
  const boundedContext = 'Hello world';
  const module = 'demo';
  const formatterConfig = null;
  const language = 'TypeScript';

  VALID_READ_MODEL_EVALUATION_TEST_CASES.forEach((testCase) => {
    it(`${testCase.description}`, () => {
      let resultCore: TTargetCoreFinalContent[];

      // given
      const tree = new IntermediateASTTree(new IntermediateASTRootNode());
      const readModelEvaluationNode = testCase.readModelEvaluation;
      tree.insertChild(readModelEvaluationNode);
