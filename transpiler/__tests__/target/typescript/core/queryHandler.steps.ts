...
import { VALID_QUERY_HANDLER_TEST_CASES } from './mocks/queryHandler/index.js';

describe('Valid queryHandler test cases', () => {
  const boundedContext = 'Hello world';
  const module = 'demo';
  const formatterConfig = null;
  const language = 'TypeScript';

  VALID_QUERY_HANDLER_TEST_CASES.forEach((testCase) => {
    it(`${testCase.description}`, () => {
      let resultCore: TTargetCoreFinalContent[];

      // given
      const tree = new IntermediateASTTree(new IntermediateASTRootNode());
      const queryHandlerNode = testCase.queryHandler;
      tree.insertChild(queryHandlerNode);
