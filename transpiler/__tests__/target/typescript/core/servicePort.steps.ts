...
import { VALID_SERVICE_PORT_TEST_CASES } from './mocks/service-port/index.js';

describe('Valid service port test cases', () => {
  const boundedContext = 'Hello world';
  const module = 'demo';
  const formatterConfig = null;
  const language = SupportedLanguages.TypeScriptNest;

  VALID_SERVICE_PORT_TEST_CASES.forEach((testCase) => {
    it(`${testCase.description}`, () => {
      let resultCore: TTargetCoreFinalContent[];

      // given
      const tree = new IntermediateASTTree(new IntermediateASTRootNode());
      const servicePortNode = testCase.servicePort;
      tree.insertChild(servicePortNode);
