...
import { VALID_DOMAIN_SERVICE_TEST_CASES } from './mocks/domain-service/index.js';

describe('Valid domain service test cases', () => {
  const boundedContext = 'Hello world';
  const module = 'demo';
  const formatterConfig = null;
  const language = SupportedLanguages.TypeScriptNest;

  VALID_DOMAIN_SERVICE_TEST_CASES.forEach((testCase) => {
    it(`${testCase.description}`, () => {
      let resultCore: TTargetCoreFinalContent[];

      // given
      const tree = new IntermediateASTTree(new IntermediateASTRootNode());
      const domainServiceNode = testCase.domainService;
      tree.insertChild(domainServiceNode);
