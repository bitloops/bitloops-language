...
import { VALID_COMMAND_HANDLER_TEST_CASES } from './mocks/commandHandler/index.js';

describe('Valid commandHandler test cases', () => {
  const boundedContext = 'Hello world';
  const module = 'demo';
  const formatterConfig = null;
  const language = 'TypeScript';

  VALID_COMMAND_HANDLER_TEST_CASES.forEach((testCase) => {
    it(`${testCase.description}`, () => {
      let resultCore: TTargetCoreFinalContent[];

      // given
      const tree = new IntermediateASTTree(new IntermediateASTRootNode());
      const commandHandlerNode = testCase.commandHandler;
      tree.insertChild(commandHandlerNode);
