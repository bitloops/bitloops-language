...
import {
  contextInfo,
  VALID_INTEGRATION_EVENT_HANDLER_TEST_CASES,
} from './mocks/integration-event-handler/index.js';

describe('Valid integration event handler declaration test cases', () => {
  const formatterConfig = null;
  const language = SupportedLanguages.TypeScriptNest;
  const { boundedContextName, moduleName } = contextInfo;

  VALID_INTEGRATION_EVENT_HANDLER_TEST_CASES.forEach((testCase) => {
    let resultCore: TTargetCoreFinalContent[];

    it(`${testCase.description}`, () => {
      // given
      const tree = new IntermediateASTTree(new IntermediateASTRootNode());
      const integrationEventHandler = testCase.integrationEventHandler;

      tree.insertChild(integrationEventHandler);
