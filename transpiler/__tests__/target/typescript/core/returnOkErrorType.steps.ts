...
import { VALID_RETURN_OK_ERROR_TYPE_TEST_CASES } from './mocks/returnOkErrorType/index.js';

describe('Return ok error type test cases', () => {
  const boundedContext = 'Hello world';
  const module = 'demo';
  const formatterConfig = null;
  const language = SupportedLanguages.TypeScriptNest;

  VALID_RETURN_OK_ERROR_TYPE_TEST_CASES.forEach((testCase) => {
    it(`${testCase.description}`, () => {
      let resultCore: TTargetCoreFinalContent[];

      // given
      const tree = new IntermediateASTTree(new IntermediateASTRootNode());
      const returnOkErrorTypeNode = testCase.returnOkErrorType;
      tree.insertChild(returnOkErrorTypeNode);
